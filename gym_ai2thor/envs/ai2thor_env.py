...
class AI2ThorEnv(gym.Env):
    ...
    def __init__(self, seed=None, config_file='config_files/config_example.json', config_dict=None):
        ...
        # Action settings
        self.action_names = tuple(list(ALL_POSSIBLE_ACTIONS))
        ...
        self.metadata_last_object_attributes = ['lastObjectPut', 'lastObjectPutReceptacle',
                                                ...
                                                'lastObjectClosed']
        self.cameraY = self.config.get('cameraY', 0.1)
        self.gridSize = self.config.get('gridSize', 0.25)
        ...
        self.event = self.controller.reset(self.scene_id)

        # Record the starting position and rotation
        self.x = self.event.metadata['agent']['position']['x']
        self.y = self.event.metadata['agent']['position']['y']
        self.z = self.event.metadata['agent']['position']['z']
        self.rotation = self.event.metadata['agent']['rotation']['y']

        # Create map
        self.AlreadyInMap = {}

        self.last_position = self.event.metadata['agent']['position']
        self.collided = False
    ...
    def step(self, action, verbose=True):
        ...
        if action_str.endswith('Object'):  # All interactions end with 'Object'
            # Interaction actions
            interaction_obj, distance = None, 1.0
        ...
        # state_image = self.event.frame

        if action_str.startswith('Move') and self.last_position == self.event.metadata['agent']['position']:
            self.collided = True

        reward, done, object_sum = self.task.transition_reward(self.event, self.AlreadyInMap, self.collided)
        #if done:
            #print('find objects num:', object_sum)
    ...
    def preprocess(self, img):
        ...
        img = np.moveaxis(img, 2, 0)   #change the state matrix from (84,84,3) to (3,84,84)
        return img
    ...
    def reset(self):
        ...
        #self.event = self.controller.step(dict(action='TeleportFull', x=-5.25, y=1, z=-1.5, rotation=90, horizon=0))
        self.AlreadyInMap = {}
        self.collided = False
        self.task.reset()
